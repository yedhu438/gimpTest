...
cur.execute("""
    ...
    AND CAST(o.DateAdd AS DATE) = '2026-06-03'
    AND CAST(o.DateAdd AS TIME) >= '13:00:00'
    ORDER BY o.DateAdd ASC
""")
rows = cur.fetchall()
conn.close()

print(f"Fetched {len(rows)} rows. Processing unique orders...\n")
...
for row in rows:
    ...
    print(f"[JOB {written+1:02d}] {oid} | {sku:<30} | {routing}")
    written += 1
