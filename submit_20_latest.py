...
conn = get_connection()
cur  = conn.cursor()
# Fetch more than 20 so we have room to skip emb/rhine orders
cur.execute("""
    SELECT TOP 60 o.OrderID, o.SKU,
...
print(f"Fetched {len(rows)} orders. Selecting 20 valid...\n")
...
for row in rows:
    ...
    if is_manual_order(row[5], row[11]):
        print(f"[SKIP-MANUAL] {oid} -- emb/rhine font")
        skipped += 1
        continue
    ...
    fonts   = list(set(d['font_family'] for d in zones.values()))
    print(f"[JOB {written+1:02d}] {oid} | SKU:{sku} | {routing} | zones:{list(zones.keys())} | fonts:{fonts}")
    written += 1
